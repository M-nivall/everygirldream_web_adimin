...
$select = "SELECT er.report_id, er.is_anonymous, er.urgency, er.county, er.town_village, er.specific_address, 
                  er.age_group, er.number_of_girls, er.description, er.emergency_status, er.assigned_team
           FROM emergency_reports er
           INNER JOIN employees e ON er.assigned_team = e.team
           WHERE er.emergency_status IN ('Assigned', 'In Progress') AND e.emp_id = '$leadID'
...
if (mysqli_num_rows($query) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $temp['assignedTeam'] = $row['assigned_team'];
