...
if ($_SERVER['REQUEST_METHOD'] =='POST') {
    ...
    if (empty($firstname) || empty($lastname) || empty($username) || empty($email) || empty($phoneNo) || empty($password)) {
        ...
        $select = "SELECT username FROM girls WHERE username='$username'";
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            $select = "SELECT phone_no FROM girls WHERE phone_no='$phoneNo'";
            $query = mysqli_query($con, $select);
            if (mysqli_num_rows($query) > 0) {
                ...
                $select = "SELECT email FROM girls WHERE email='$email'";
                $query = mysqli_query($con, $select);
                if (mysqli_num_rows($query) > 0) {
                    ...
                    $insert = "INSERT INTO girls(first_name, last_name, username, phone_no, email, password, age, school, location)
                VALUES ('$firstname','$lastname','$username','$phoneNo','$email','$password','$age','$school','$location')";
